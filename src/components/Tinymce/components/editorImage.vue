<template>
  <div class="upload-container">
    ...
    <el-dialog append-to-body :visible.sync="dialogVisible">
      ...
      <el-upload ref="upload" class="editor-slide-upload" action="http://upload.qiniup.com" :multiple="false" :file-list="fileList" :show-file-list="true" 
       list-type="picture-card" :on-remove="handleRemove" :on-success="handleSuccess" :before-upload="beforeUpload" :data="form">
            <el-button size="small" type="primary">点击上传</el-button>
          </el-upload>
...
export default {
  ...
  methods: {
    ...
    handleSuccess(response, file) {
      ...
      for (let i = 0, len = objKeyArr.length; i < len; i++) {
        if (this.listObj[objKeyArr[i]].uid === uid) {
          this.listObj[objKeyArr[i]].url = iconAddress
          this.listObj[objKeyArr[i]].hasSuccess = true
          return
        }
      }
    ...
    beforeUpload(file) {
      ...
      // const suffix = file.name //_${suffix}
