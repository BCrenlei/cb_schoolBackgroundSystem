      ...
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="130px" style='width: 500px; margin-left:50px;'>
        ...
        <el-form-item :label="$t('new.selfFeeling')" prop="selfFeeling">
          <el-input
            ...
            placeholder="请输入内容"
            v-model="temp.selfFeeling">
          </el-input>
        </el-form-item>

        <el-form-item :label="$t('new.selfFeelingEn')" prop="selfFeelingEn">
          <el-input
            type="textarea"
            :autosize="{ minRows: 2, maxRows: 6}"
            placeholder="请输入内容"
            v-model="temp.selfFeelingEn">
          </el-input>
        </el-form-item>
...
import { listStEvaluation, myListStEvaluation, addSEvaluation, updateStEvaluation, deleteStEvaluation } from '@/api/assessM'
...
export default {
  ...
  methods: {
    ...
    configOperation(row) {
      ...
      if (this.teachingGradeId != '' && this.teachingCoursesId != '') {
        if (row.gradeId == this.teachingGradeId && row.userCourseId == this.teachingCoursesId) {
          judge = true
        }
      }
      return judge
    ...
    configTemp(row) {
      this.temp = {
        ...
        selfFeelingEn: row.selfFeelingEn
      }
    },
    // 添加 的时候 下拉框 需要的参数
    configAddOptions() {
